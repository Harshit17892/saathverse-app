...
import {
  ...
  Check, X, Loader2, Plus, Search, Trash2,
  Save
} from "lucide-react";
...
const ClubDashboardSection = () => {
  ...
  // Load all clubs for this college
  useEffect(() => {
    if (!collegeId) return;
    const load = async () => {
  ...
  if (allClubs.length === 0) {
    return (
      <div className="glass rounded-2xl p-12 text-center">
        ...
        <p className="text-muted-foreground">No clubs found for this college. Create clubs in the Clubs section first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <select
          className="bg-secondary/50 border border-border/30 rounded-lg px-3 py-2 text-sm text-foreground min-w-[200px]"
          value={selectedClubId || ""}
          onChange={(e) => { setSelectedClubId(e.target.value); setActiveTab("overview"); }}
        >
          {allClubs.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        {club && <Badge variant="outline" className="text-accent border-accent/30">{club.category || "Club"}</Badge>}
      </div>
